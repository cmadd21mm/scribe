...
/// Manage scribe's LaunchAgent so the daemon starts at login.
///
/// We deliberately do NOT use SMAppService.mainApp here — that requires a full
/// .app bundle. Since scribe ships as a single binary in /usr/local/bin, a
...
struct Install: ParsableCommand {
    ...
    @Flag(name: .long, help: "Register scribe to start at login.")
    ...
    private static let label = "com.cmadd21mm.scribe"
    ...
    private func writeAgent() throws {
        ...
        let plist: [String: Any] = [
            ...
            "StandardOutPath": "/tmp/scribe.out.log",
            "StandardErrorPath": "/tmp/scribe.err.log",
        ]
        ...
        print("  binary: \(binary)")
        print("  logs:   /tmp/scribe.out.log, /tmp/scribe.err.log")
    }
    ...
    private func resolveBinaryPath() throws -> String {
        // /usr/local/bin/scribe is the canonical install path. Honor a real
        // location if running from elsewhere (e.g. dev).
        let candidate = "/usr/local/bin/scribe"
        if FileManager.default.isExecutableFile(atPath: candidate) {
            return candidate
        }
        // Fall back to the running executable's resolved path.
        let argv0 = CommandLine.arguments.first ?? "scribe"
        if argv0.hasPrefix("/"), FileManager.default.isExecutableFile(atPath: argv0) {
            FileHandle.standardError.write(Data(
                "note: /usr/local/bin/scribe not found; using \(argv0)\n".utf8
            ))
            return argv0
        }
        FileHandle.standardError.write(Data(
            "couldn't locate the scribe binary. install it to /usr/local/bin/scribe first.\n".utf8
        ))
        throw ExitCode(1)
    }
